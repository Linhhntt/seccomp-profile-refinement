                ...
                if ( (not options.singleappname and appDict.get("enable","true") == "true") or (options.singleappname and appName == options.singleappname) ):
                    ...
                    if ( not options.libdebloating ):
                        ...
                        if ( not os.path.exists(output) ):
                            ...
                            piecewiseWorkerSyscalls = []
                            piecewiseMasterSyscalls = []
                            ...
                            temporalObj = piecewise.Piecewise(appName, binbasepath + "/" + bininput + "/", cfgbasepath + "/" + temporaltargetcfg, cfginput, options.othercfgpath, rootLogger, options.cfginputseparator)
                            mainList = [masterMainList, workerMainList]
                            temporalMasterSyscalls, temporalWorkerSyscalls = temporalObj.extractAccessibleSystemCalls(masterMainList, workerMainList, exceptList)
                            rootLogger.info("Finished extracting temporal worker system calls with len: %d and %d for %s", len(temporalMasterSyscalls), len(temporalWorkerSyscalls), appName)
